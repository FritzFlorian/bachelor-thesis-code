"""Client and helper functions for the NNServer.

This is helpful to create, manage and use neural networks running in a cpython
process from other processes (e.g. a pypy process).
...
class NeuralNetworkClient:
    ...
    def evaluate_game_state(self, game_state: GameState) -> Evaluation:
    ...
    def start(self):
        ...
        self.input_conversion = response.input_conversion
        self.output_conversion = response.output_conversion
...
def _start_nn_server_internal(port, nn_class_name, batch_size, nn_init_args):
    import pure_mcts_reinforcement.nn_server as nn_server
    ...
    server = nn_server.NeuralNetworkServer(port, nn_class(nn_init_args), None, None, batch_size)
    server.run()
