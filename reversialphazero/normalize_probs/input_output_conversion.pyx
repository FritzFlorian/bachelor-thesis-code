...
def input(evaluation, calculate_target=False):
    ...
    inversion_field = Field.INVERSION
...
def output(evaluation, output_array):
    ...
    filter = np.zeros(NN_BOARD_SIZE * NN_BOARD_SIZE)
    for move, prob in evaluation.get_move_probabilities().items():
        # Only let valid moves pass
        x, y = move.internal_tuple[1]
        filter[(y + embed_y) * NN_BOARD_SIZE + (x + embed_x)] = 1
    ...
    for move, prob in evaluation.probabilities.items():
        x, y = move.internal_tuple[1]
        evaluation.probabilities[move] = output_array_probabilities[(y + embed_y) * NN_BOARD_SIZE + (x + embed_x)]
