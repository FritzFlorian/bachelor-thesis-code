...
import reinforcement.multiple_maps.input_output_conversion as input_output_conversion
...
L2_LOSS_WEIGHT = 0.004  # L2 loss was doubled (want to be conservative, as this is the first run on the new hardware)
...
class SimpleNeuralNetwork(neural_network.NeuralNetwork):
    ...
    def construct_network(self, sess, graph):
        ...
        with tf.variable_scope('Convolutional-Layers'):
            ...
            res1 = self._construct_residual_block(conv1, self.n_conv_filetrs, 'res1', dense_skip_input=self.one_hot_x_float)
            res2 = self._construct_residual_block(res1, self.n_conv_filetrs, 'res2', dense_skip_input=self.one_hot_x_float)
            res3 = self._construct_residual_block(res2, self.n_conv_filetrs, 'res3', dense_skip_input=self.one_hot_x_float)
            res4 = self._construct_residual_block(res3, self.n_conv_filetrs, 'res4', dense_skip_input=self.one_hot_x_float)
            res5 = self._construct_residual_block(res4, self.n_conv_filetrs, 'res5', dense_skip_input=self.one_hot_x_float)
            res6 = self._construct_residual_block(res5, self.n_conv_filetrs, 'res6', dense_skip_input=self.one_hot_x_float)
            res7 = self._construct_residual_block(res6, self.n_conv_filetrs, 'res7', dense_skip_input=self.one_hot_x_float)
            res8 = self._construct_residual_block(res7, self.n_conv_filetrs, 'res8', dense_skip_input=self.one_hot_x_float)
            res8_with_dense_skip = tf.concat([self.one_hot_x_float, res8], 3)

        with tf.variable_scope('Probability-Head'):
            n_filters = 2

            # Reduce the big amount of convolutional filters to a reasonable size.
            prob_conv = self._construct_conv_layer(res8_with_dense_skip, n_filters, 'prob_conv', kernel=[1, 1], stride=1)
        ...
        with tf.variable_scope('Value-Head'):
            ...
            # Reduce the big amount of convolutional filters to a reasonable size.
            value_conv = self._construct_conv_layer(res8_with_dense_skip, n_filters, 'value_conv', kernel=[1, 1], stride=1)
            # Flattern the output tensor to allow it as input to a fully connected layer.
